...
function getLocalString(date) {
  ...
  return `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())} ${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())} GMT${tz < 0 ? '+' : '-'}${pad(Math.abs(Math.floor(tz) / 60))}:${pad(tz % 60)}`
}
